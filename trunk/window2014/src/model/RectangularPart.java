...
public class RectangularPart extends RectangularArea {
	...
	void setParent(RectangularArea parent) throws HierarchyException {
		...
		if (!(pointParent.getX() <= pointPart.getX()
				&& pointParent.getX() + parent.getWidth() >= pointPart.getX()
						+ this.getWidth()
				&& pointParent.getY() >= pointPart.getY()
				&& pointParent.getY() + parent.getHeight() <= pointPart.getY()
						+ this.getHeight())) {
			throw new Error();
		} else {
			if (parent.isIn(this))
	...
	public boolean doNotOverlap(RectangularPart rectangularPart){
		...
		if (point1.getX()>point2.getX()+this.getWidth() || point1.getX()+rectangularPart.getWidth()<point2.getX()) {
			return true;
		}
		return point1.getY()<point2.getY()-this.getHeight() || point1.getY()-rectangularPart.getHeight()>point2.getY();
	}
}
