...
public class View extends JFrame {
	...
	private JButton substituteButton = null;
	...
	protected void select_action(final boolean first) {
		final Expression selected = (Expression) this.getCellList().getSelectedValue();
		if (selected != null){
	...
	private JToolBar getUpDownToolBar() {
		if (upDownToolBar == null) {
			...
			upDownToolBar.add(getSubstituteButton());
	...
	protected void variable_action(final boolean b) {
		final Variable selected = (Variable) this.getVariableList().getSelectedValue();	
		if (selected != null){
	...
	/**
	 * This method initializes substituteButton	
	 * 	
	 * @return javax.swing.JButton	
	 */
	private JButton getSubstituteButton() {
	...
	protected void substitute_action() {
		final Object selectedVariableObject = this.getVariableList().getSelectedValue();
		final Object selectedExpressionObject = this.getCellList().getSelectedValue();
		if (selectedExpressionObject != null){
			final Expression selectedExpression = (Expression) selectedExpressionObject; 
			if (selectedVariableObject != null){
				final Variable selectedVariable = (Variable) selectedVariableObject;
				this.getFacade().substitute(selectedVariable,selectedExpression);
				this.refresh();
			}else{
				this.variableStatusLabel.setText("Select varibale!");
			}
		}else{
			this.expressionStatusLabel.setText("Select expression");
		}
